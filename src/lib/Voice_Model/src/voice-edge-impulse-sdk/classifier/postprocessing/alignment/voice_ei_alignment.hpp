...
float intersection_over_union(const voice_ei_impulse_result_bounding_box_t bbox1, const voice_ei_impulse_result_bounding_box_t bbox2) {
...
float centroid_euclidean_distance(const voice_ei_impulse_result_bounding_box_t bbox1, const voice_ei_impulse_result_bounding_box_t bbox2) {
    ...
    VOICE_EI_LOGD("centroid_euclidean_distance: x1=%f y1=%f x2=%f y2=%f distance=%f\n", x1, y1, x2, y2, distance);
    return distance;
}
...
public:
    ...
    std::vector<std::tuple<int, int, float>> align(const std::vector<voice_ei_impulse_result_bounding_box_t> traces,
                                                   const std::vector<voice_ei_impulse_result_bounding_box_t> detections) {
        ...
        for (size_t trace_idx = 0; trace_idx < traces.size(); ++trace_idx) {
            for (size_t detection_idx = 0; detection_idx < detections.size(); ++detection_idx) {
                ...
                VOICE_EI_LOGD("t_idx=%zu d_idx=%zu cost=%.6f\n", trace_idx, detection_idx, cost);
        ...
        VOICE_EI_LOGD("detections size %zu\n", detections.size());
        VOICE_EI_LOGD("traces size %zu\n", traces.size());

        for (size_t i = 0; i < traces.size(); i++) {
            VOICE_EI_LOGD("alignments_a[%zu] %lld\n", i, alignments_a[i]);
        }

        for (size_t i = 0; i < detections.size(); i++) {
            VOICE_EI_LOGD("alignments_b[%zu] %lld\n", i, alignments_b[i]);
        }
...
public:
    ...
    std::vector<std::tuple<int, int, float>> align(const std::vector<voice_ei_impulse_result_bounding_box_t> traces,
        ...
        VOICE_EI_LOGD("alignments.size() %zu\n", alignments.size());
        std::vector<std::tuple<int, int, float>> matches;
