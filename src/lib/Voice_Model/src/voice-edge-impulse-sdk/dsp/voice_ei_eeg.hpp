 ...
 */
#ifndef VOICE_EI_EEG_HPP
#define VOICE_EI_EEG_HPP

#include "voice-edge-impulse-sdk/dsp/voice_ei_dsp_handle.h"
#include "edge-impulse-enterprise/eeg/eeg.hpp"

// Include the generated header
#include "voice-model-parameters/coeffs.hpp"

// Wrapper to use voice_ei_malloc for allocation
...
public:
    int print() override {
        voice_ei_printf("EEG processor: channels=%zu\n", eeg.num_channels);
        return ei::EIDSP_OK;
    }
        ...
        voice_ei_impulse_result_t *result) override
    {
        ...
        voice_ei_dsp_config_eeg_t& config = *((voice_ei_dsp_config_eeg_t*)config_ptr);
    ...
    eeg_class(voice_ei_dsp_config_eeg_t* config, float frequency)
        : eeg(frequency,
              config->axes,
              config->scale_axes,
...
DspHandle* eeg_class::create(void* config_in, float frequency) {
    auto config = reinterpret_cast<voice_ei_dsp_config_eeg_t*>(config_in);
    return new eeg_class(config, frequency);
}

#endif // VOICE_EI_EEG_HPP
