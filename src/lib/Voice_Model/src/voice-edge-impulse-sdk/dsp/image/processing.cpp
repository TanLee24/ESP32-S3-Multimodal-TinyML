 ...
 */
#include "voice-edge-impulse-sdk/dsp/image/processing.hpp"
...
#define VOICE_EI_CLAMP(t) (((t) > 255) ? 255 : (((t) < 0) ? 0 : (t)))

// Color space conversion for RGB
#define VOICE_EI_GET_R_FROM_YUV(y, u, v) ((298 * y + 409 * v + 128) >> 8)
#define VOICE_EI_GET_G_FROM_YUV(y, u, v) ((298 * y - 100 * u - 208 * v + 128) >> 8)
#define VOICE_EI_GET_B_FROM_YUV(y, u, v) ((298 * y + 516 * u + 128) >> 8)
    ...
    for (unsigned int i = 0; i < in_size_pixels; ++i) {
        ...
        if (TEST_BIT_MASK(opts, BIG_ENDIAN_ORDER)) {
            *rgb_out-- = VOICE_EI_CLAMP(VOICE_EI_GET_B_FROM_YUV(y2, u0, v));
            *rgb_out-- = VOICE_EI_CLAMP(VOICE_EI_GET_G_FROM_YUV(y2, u0, v));
            *rgb_out-- = VOICE_EI_CLAMP(VOICE_EI_GET_R_FROM_YUV(y2, u0, v));
            if (TEST_BIT_MASK(opts, PAD_4B)) {
                *rgb_out-- = 0;
            }

            *rgb_out-- = VOICE_EI_CLAMP(VOICE_EI_GET_B_FROM_YUV(y0, u0, v));
            *rgb_out-- = VOICE_EI_CLAMP(VOICE_EI_GET_G_FROM_YUV(y0, u0, v));
            *rgb_out-- = VOICE_EI_CLAMP(VOICE_EI_GET_R_FROM_YUV(y0, u0, v));
            if (TEST_BIT_MASK(opts, PAD_4B)) {
                *rgb_out-- = 0;
            }
        }
...
int resize_image(
    ...
    int pixel_size_B)
{
    // Copied from voice_ei_camera.cpp in firmware-eta-compute
    // Modified for RGB888
    ...
    int mode)
{
    ...
    if (srcWidth == dstWidth && srcHeight == dstHeight) {
        VOICE_EI_LOGI("Source and destination sizes are the same. No resizing needed.\n");
    ...
    if (srcImage == dstImage && mode == VOICE_EI_CLASSIFIER_RESIZE_FIT_LONGEST) {
        VOICE_EI_LOGI("FIT LONGEST in place, make sure source is oversized to fit destination size\n");
    }

    if (mode == VOICE_EI_CLASSIFIER_RESIZE_FIT_SHORTEST) {
        ...
        if (res != 0) {
            VOICE_EI_LOGE("Error in crop_and_interpolate_image: %d\n", res);
            return res;
        }
        return 0;
    }

    if (mode == VOICE_EI_CLASSIFIER_RESIZE_SQUASH) {
        int res =
            resize_image(srcImage, srcWidth, srcHeight, dstImage, dstWidth, dstHeight, pixel_size_B);

        if (res != 0) {
            VOICE_EI_LOGE("Error in resize_image: %d\n", res);
            return res;
        }
        return 0;
    }

    if (mode == VOICE_EI_CLASSIFIER_RESIZE_FIT_LONGEST) {
