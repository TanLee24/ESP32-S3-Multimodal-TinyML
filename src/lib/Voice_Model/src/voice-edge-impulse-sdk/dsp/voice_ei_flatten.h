 ...
 */
#ifndef __VOICE_EI_FLATTEN__H__
#define __VOICE_EI_FLATTEN__H__
...
class voice_flatten_class : public DspHandle {
public:
    int print() override {
        voice_ei_printf("means: ");
        for(int axis = 0; (size_t)axis < this->means.size(); axis++) {
        voice_ei_printf("axis: %i\n", axis);
            for (size_t i = 0; i < this->means.size(); i++) {
                voice_ei_printf("%f ", this->means[axis][i]);
            }
        }
        voice_ei_printf("\n");
        return ei::EIDSP_OK;
    ...
    int extract(
        ...
        voice_ei_impulse_result_t *result) override
    {
        using namespace ei;

        voice_ei_dsp_config_flatten_t config = *((voice_ei_dsp_config_flatten_t*)config_ptr);
...
private:
    voice_ei_vector<voice_ei_vector<float>> means;
    voice_ei_vector<size_t> head_indexes;
    size_t moving_avg_num_windows;

    voice_flatten_class(int moving_avg_num_windows, int axes_count) : means(axes_count), head_indexes(axes_count, 0) {
...
DspHandle* voice_flatten_class::create(void* config_in, float _sampling_frequency) { // NOLINT def in header is OK at EI
    auto config = reinterpret_cast<voice_ei_dsp_config_flatten_t*>(config_in);
    return new voice_flatten_class(config->moving_avg_num_windows, config->axes);
};

#endif  //!__VOICE_EI_FLATTEN__H__