...
class kissfft
{
    public:
                     ...
                     const bool inverse )
        {
            ...
            else if ( inverse != _inverse )
            {
                ...
                for ( typename voice_ei_vector<cpx_t>::iterator it = _twiddles.begin();
                      it != _twiddles.end(); ++it )
                    it->imag( -it->imag() );
            }
        }
    ...
    private:
        ...
        std::size_t _nfft;
        bool _inverse;
        voice_ei_vector<cpx_t> _twiddles;
        voice_ei_vector<std::size_t> _stageRadix;
        voice_ei_vector<std::size_t> _stageRemainder;
        mutable voice_ei_vector<cpx_t> _scratchbuf;
};
#endif
