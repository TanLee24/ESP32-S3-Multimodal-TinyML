...
#ifndef VOICE_EI_ESP_DSP_H
#define VOICE_EI_ESP_DSP_H

#include <stdint.h>
#include <stdbool.h>
#include "voice-edge-impulse-sdk/porting/espressif/esp-dsp/modules/fft/include/dsps_fft2r.h"
...
static bool init_fft(size_t n_fft) {
    ...
    VOICE_EI_LOGD("Initializing ESP-DSP FFT with size %zu\n", n_fft);
    if (!can_do_fft(n_fft)) { return false; /* EIDSP_FFT_SIZE_NOT_SUPPORTED */ }

    esp_err_t ret = dsps_fft2r_init_fc32(NULL, CONFIG_DSP_MAX_FFT_SIZE);
    if (ret != ESP_OK) {
        VOICE_EI_LOGE("Not possible to initialize FFT. Error = %i\n", ret);
        return false;
    }
    return true;
}
...
static int hw_r2c_fft(const float *input, ei::fft_complex_t *output_as_complex, size_t n_fft) {
    if (!init_done) {
        if (!init_fft(n_fft)) {
            VOICE_EI_LOGE("Failed to initialize FFT\n");
    ...
    float *complex_input = (float*)voice_ei_malloc(n_fft * sizeof(float) * 2);
    if (complex_input == nullptr) {
        VOICE_EI_LOGE("Failed to allocate memory for complex input\n");
        goto out;
    ...
    if (err != 0) {
        VOICE_EI_LOGE("Error in dsps_fft2r_fc32: %d\n", err);
        return err;
    }
