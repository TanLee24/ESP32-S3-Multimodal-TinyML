...
// Need a wrapper to get voice_ei_malloc used
...
public:
        ...
        voice_ei_impulse_result_t *result) override
    {
        ...
        voice_ei_dsp_config_hr_t& config = *((voice_ei_dsp_config_hr_t*)config_ptr);
    ...
    hr_class(voice_ei_dsp_config_hr_t* config, float frequency)
        : ppg(frequency,
              config->axes,
              ...
              true), hrv(nullptr)
    {
        ...
        if(strcmp(config->hrv_features,"none") != 0) {
            // new is overloaded to use voice_ei_malloc
            hrv = new hrv_wrap(
                frequency,
                config->hrv_features,
    ...
    ~hr_class() {
        if(hrv) {
            // delete is overloaded to use voice_ei_free
            delete hrv;
        }
    }
...
DspHandle* hr_class::create(void* config_in, float frequency) { // NOLINT def in header is OK at EI
    auto config = reinterpret_cast<voice_ei_dsp_config_hr_t*>(config_in);
