...
typedef struct voice_ei_stack_frames_info {
    signal_t *signal;
    voice_ei_vector<uint32_t> frame_ixs;
    int frame_length;
} stack_frames_info_t;
...
namespace processing {
    ...
    class preemphasis {
public:
        preemphasis(voice_ei_signal_t *signal, int shift, float cof, bool rescale)
            : _signal(signal), _shift(shift), _cof(cof), _rescale(rescale)
        {
            _prev_buffer = (float*)voice_ei_dsp_calloc(shift * sizeof(float), 1);
            _end_of_signal_buffer = (float*)voice_ei_dsp_calloc(shift * sizeof(float), 1);
        ...
        ~preemphasis() {
            if (_prev_buffer) {
                voice_ei_dsp_free(_prev_buffer, _shift * sizeof(float));
            }
            if (_end_of_signal_buffer) {
                voice_ei_dsp_free(_end_of_signal_buffer, _shift * sizeof(float));
            }
        }
...
namespace processing {
    ...
    __attribute__((unused)) static int preemphasis(float *signal, size_t signal_size, int shift = 1, float cof = 0.98f)
    {
        ...
        float *prev_buffer = (float*)voice_ei_dsp_calloc(shift * sizeof(float), 1);
        ...
        voice_ei_dsp_free(prev_buffer, shift * sizeof(float));
        ...
        bool scale = false)
    {
        ...
        VOICE_EI_DSP_MATRIX(vec_pad, features_matrix->rows + (pad_size * 2), features_matrix->cols);
        if (!vec_pad.buffer) {
            EIDSP_ERR(EIDSP_OUT_OF_MEM);
        }
        ...
        VOICE_EI_DSP_MATRIX(mean_matrix, vec_pad.cols, 1);
        if (!mean_matrix.buffer) {
            EIDSP_ERR(EIDSP_OUT_OF_MEM);
        }

        VOICE_EI_DSP_MATRIX(window_variance, vec_pad.cols, 1);
        ...
        for (size_t ix = 0; ix < features_matrix->rows; ix++) {
            ...
            VOICE_EI_DSP_MATRIX_B(window, win_size, vec_pad.cols, vec_pad.buffer + (ix * vec_pad.cols));
            if (!window.buffer) {
                EIDSP_ERR(EIDSP_OUT_OF_MEM);
            }
