...
public:
        ...
        )
    {
        ...
        float *mels = (float*)voice_ei_dsp_malloc(mels_mem_size);
        if (!mels) {
        ...
        float *hertz = (float*)voice_ei_dsp_malloc(hertz_mem_size);
        if (!hertz) {
            voice_ei_dsp_free(mels, mels_mem_size);
            EIDSP_ERR(EIDSP_OUT_OF_MEM);
        }
        ...
        int *freq_index = (int*)voice_ei_dsp_malloc(freq_index_mem_size);
        if (!freq_index) {
            voice_ei_dsp_free(hertz, hertz_mem_size);
            EIDSP_ERR(EIDSP_OUT_OF_MEM);
        }
        for (uint16_t ix = 0; ix < num_filter + 2; ix++) {
        ...
        for (size_t i = 0; i < num_filter; i++) {
            ...
            VOICE_EI_DSP_MATRIX(z, 1, (right - left + 1));
            if (!z.buffer) {
                voice_ei_dsp_free(freq_index, freq_index_mem_size);
                EIDSP_ERR(EIDSP_OUT_OF_MEM);
            }
        ...
        )
    {
        ...
        mels = (float*)voice_ei_dsp_calloc(MELS_SIZE, sizeof(float));
        VOICE_EI_ERR_AND_RETURN_ON_NULL(mels, EIDSP_OUT_OF_MEM);
        voice_ei_unique_ptr_t __ptr__(mels,[mem_size](void* ptr){ei::voice_ei_dsp_free_func(ptr, mem_size);});
        ...
        VOICE_EI_DSP_MATRIX(power_spectrum_frame, 1, power_spectrum_frame_size);
        if (!power_spectrum_frame.buffer) {
            EIDSP_ERR(EIDSP_OUT_OF_MEM);
        }

        // get signal data from the audio file
        VOICE_EI_DSP_MATRIX(signal_frame, 1, stack_frame_info.frame_length);
...
#if EIDSP_QUANTIZE_FILTERBANK
        VOICE_EI_DSP_QUANTIZED_MATRIX(filterbanks, num_filters, coefficients, &numpy::dequantize_zero_one);
#else
        VOICE_EI_DSP_MATRIX(filterbanks, num_filters, coefficients);
#endif
        ...
        uint16_t version)
    {
        ...
        VOICE_EI_DSP_MATRIX(features_matrix, mfe_matrix_size.rows, mfe_matrix_size.cols);
        if (!features_matrix.buffer) {
            EIDSP_ERR(EIDSP_OUT_OF_MEM);
        }

        VOICE_EI_DSP_MATRIX(energy_matrix, mfe_matrix_size.rows, 1);
