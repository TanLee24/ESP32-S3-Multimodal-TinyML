...
namespace processing {
    ...
    class scale {
public:
        scale(voice_ei_signal_t *signal, float scaling = 1.0f)
        ...
        int get_data(size_t offset, size_t length, float *out_buffer) {
            ...
            VOICE_EI_DSP_MATRIX_B(temp, 1, length, out_buffer);
            return numpy::scale(&temp, _scaling);
        }

private:
        voice_ei_signal_t *_signal;
        float _scaling;
    };
}
...
namespace processing {
    ...
    {
        VOICE_EI_DSP_MATRIX_B(temp, 1, signal_size, signal);
        ...
        uint16_t fft_length)
    {
        ...
        VOICE_EI_DSP_MATRIX(freq_space, 1, fft_matrix->cols);
        ret = numpy::linspace(0.0f, 1.0f / (2.0f * T), floor(N / 2), freq_space.buffer);
        if (ret != EIDSP_OK) {
            EIDSP_ERR(ret);
        }

        VOICE_EI_DSP_MATRIX(peaks_matrix, output_matrix->rows * 10, 1);
        ...
        voice_ei_vector<freq_peak_t> peaks;
    ...
    int spectral_power_edges(
        ...
        VOICE_EI_DSP_MATRIX(buckets, 1, edges_matrix->rows - 1);
        VOICE_EI_DSP_MATRIX(bucket_count, 1, edges_matrix->rows - 1);
    ...
    int periodogram(matrix_t *input_matrix, matrix_t *out_fft_matrix, matrix_t *out_freq_matrix, float sampling_freq, uint16_t n_fft)
    {
        ...
        VOICE_EI_DSP_MATRIX_B(welch_matrix, input_matrix->rows, input_matrix->cols, input_matrix->buffer);
        ...
        VOICE_EI_DSP_MATRIX(triage_segments, 1, nperseg);
        ...
        VOICE_EI_DSP_MATRIX(mean_matrix, 1, 1);
        ...
        fft_complex_t *fft_output = (fft_complex_t*)voice_ei_dsp_calloc((n_fft / 2 + 1) * sizeof(fft_complex_t), 1);
        ret = numpy::rfft(welch_matrix.buffer, welch_matrix.cols, fft_output, n_fft / 2 + 1, n_fft);
        if (ret != EIDSP_OK) {
            voice_ei_dsp_free(fft_output, (n_fft / 2 + 1) * sizeof(fft_complex_t));
            EIDSP_ERR(ret);
        }
    ...
    static int subtract_mean(matrix_t* input_matrix) {
        // calculate the mean
        VOICE_EI_DSP_MATRIX(mean_matrix, input_matrix->rows, 1);
