...
#include "wavelet.hpp"
#include "signal.hpp"
#include "voice-edge-impulse-sdk/dsp/voice_ei_utils.h"
...
public:
    ...
    static int spectral_analysis(
        ...
        VOICE_EI_TRY(processing::subtract_mean(input_matrix) );
        ...
        // calculate RMS
        VOICE_EI_DSP_MATRIX(rms_matrix, axes, 1);
        ...
        // find peaks in FFT
        VOICE_EI_DSP_MATRIX(peaks_matrix, axes, fft_peaks * 2);

        for (size_t row = 0; row < input_matrix->rows; row++) {
            // per axis code

            // get a slice of the current axis
            VOICE_EI_DSP_MATRIX_B(axis_matrix, 1, input_matrix->cols, input_matrix->buffer + (row * input_matrix->cols));

            // calculate FFT
            VOICE_EI_DSP_MATRIX(fft_matrix, 1, fft_length / 2 + 1);
            ...
            VOICE_EI_DSP_MATRIX(peaks_matrix, fft_peaks, 2);
            ...
            VOICE_EI_DSP_MATRIX(period_fft_matrix, 1, fft_length / 2 + 1);
            VOICE_EI_DSP_MATRIX(period_freq_matrix, 1, fft_length / 2 + 1);
            ...
            VOICE_EI_DSP_MATRIX(edges_matrix_out, edges_matrix_in->rows - 1, 1);
    ...
    static int extract_spectral_analysis_features_v1(
        ...
        voice_ei_dsp_config_spectral_analysis_t *config_ptr,
        const float sampling_freq)
    {
        // scale the signal
        int ret = numpy::scale(input_matrix, config_ptr->scale_axes);
        if (ret != EIDSP_OK) {
            voice_ei_printf("ERR: Failed to scale signal (%d)\n", ret);
            EIDSP_ERR(ret);
        }
        ...
        if (ret != EIDSP_OK) {
            voice_ei_printf("ERR: Failed to transpose matrix (%d)\n", ret);
            EIDSP_ERR(ret);
        }
        ...
        // voice_ei_printf("output_matrix_size %hux%zu\n", input_matrix.rows, output_matrix_cols);
        ...
        ret = spectral::feature::spectral_analysis(
            ...
            &edges_matrix_in);
        if (ret != EIDSP_OK) {
            voice_ei_printf("ERR: Failed to calculate spectral features (%d)\n", ret);
            EIDSP_ERR(ret);
        }
    ...
    static size_t extract_spec_features(
        matrix_t *input_matrix,
        matrix_t *output_matrix,
        voice_ei_dsp_config_spectral_analysis_t *config,
        const float sampling_freq,
        ...
        const bool transpose_and_scale_input = true)
    {
        if (transpose_and_scale_input) {
            ...
            VOICE_EI_TRY(numpy::scale(input_matrix, config->scale_axes));
        }
        ...
        if (strcmp(config->filter_type, "low") == 0) {
            if( config->filter_order ) {
                VOICE_EI_TRY(spectral::processing::butterworth_lowpass_filter(
                    input_matrix,
                    sampling_freq,
            ...
            if( config->filter_order ) {
                VOICE_EI_TRY(spectral::processing::butterworth_highpass_filter(
                    input_matrix,
                    sampling_freq,
                    config->filter_cutoff,
                    config->filter_order));
        ...
        for (size_t row = 0; row < input_matrix->rows; row++) {
            ...
            VOICE_EI_TRY(numpy::rms(&rms_in_matrix, &rms_out_matrix));
            ...
            *feature_out++ = ((k_sum / data_size) / (temp * stddev)) - 3;
            ...
            if (config->implementation_version == 4) {
                ...
                voice_ei_vector<float> fft_out(fft_out_size);
                VOICE_EI_TRY(numpy::welch_max_hold(
                    data_window,
                    data_size,
                    fft_out.data(),
                    0,
            ...
            if (config->do_log) {
                ...
                voice_ei_matrix temp(num_bins, 1, feature_out);
                numpy::log10(&temp);
            }
    ...
    static voice_ei_vector<int> get_ratio_combo(int r)
    {
        if (r == 1 || r == 3 || r == 10) {
            return {r};
        } else if (r == 30) {
    ...
    static int extract_spectral_analysis_features_v4(
        matrix_t *input_matrix,
        matrix_t *output_matrix,
        voice_ei_dsp_config_spectral_analysis_t *config_p,
        const float sampling_freq)
    {
        ...
        if (strcmp(config->analysis_type, "Wavelet") == 0) {
            ...
            if (config->input_decimation_ratio > 1) {
                voice_ei_vector<int> ratio_combo = get_ratio_combo(config->input_decimation_ratio);
