 ...
 */
typedef struct voice_ei_matrix {
    float *buffer;
    uint32_t rows;
    uint32_t cols;
        ...
        )
    {
        if (a_buffer) {
            ...
            buffer = (float*)voice_ei_calloc(n_rows * n_cols * sizeof(float), 1);
        ...
        if (!a_buffer) {
            ...
            if (_fn) {
                voice_ei_dsp_register_matrix_alloc_internal(fn, file, line, rows, cols, sizeof(float), buffer);
            }
            else {
                voice_ei_dsp_register_matrix_alloc(rows, cols, sizeof(float), buffer);
            }
#endif
        }
    }
    ...
    ~voice_ei_matrix() {
        if (buffer && buffer_managed_by_me) {
            voice_ei_free(buffer);

#if EIDSP_TRACK_ALLOCATIONS
            if (_fn) {
                voice_ei_dsp_register_matrix_free_internal(_fn, _file, _line, _originally_allocated_rows,
                    _originally_allocated_cols, sizeof(float), buffer);
            }
            else {
                voice_ei_dsp_register_matrix_free(_originally_allocated_rows, _originally_allocated_cols,
                    sizeof(float), buffer);
            }
#endif
        }
    ...
    voice_ei_matrix(voice_ei_vector<float> &in) : voice_ei_matrix(1, in.size(), in.data()) {
    }
 ...
 * Freeing happens by calling `delete` on the object or letting the object go out of scope.
 */
typedef struct voice_ei_matrix_i8 {
    int8_t *buffer;
    uint32_t rows;
    uint32_t cols;
    bool buffer_managed_by_me;
        ...
        )
    {
        if (a_buffer) {
            ...
            buffer = (int8_t*)voice_ei_calloc(n_rows * n_cols * sizeof(int8_t), 1);
            buffer_managed_by_me = true;
        }
        rows = n_rows;
        cols = n_cols;
        ...
        if (!a_buffer) {
            ...
            _originally_allocated_cols = cols;
            if (_fn) {
                voice_ei_dsp_register_matrix_alloc_internal(fn, file, line, rows, cols, sizeof(int8_t), buffer);
            }
            else {
                voice_ei_dsp_register_matrix_alloc(rows, cols, sizeof(int8_t), buffer);
            }
#endif
        }
    }
 ...
 * Freeing happens by calling `delete` on the object or letting the object go out of scope.
 */
typedef struct voice_ei_matrix_i32 {
    int32_t *buffer;
    uint32_t rows;
    uint32_t cols;
    bool buffer_managed_by_me;
        ...
        )
    {
        if (a_buffer) {
            ...
            buffer = (int32_t*)voice_ei_calloc(n_rows * n_cols * sizeof(int32_t), 1);
            buffer_managed_by_me = true;
        }
        rows = n_rows;
        cols = n_cols;
        ...
        if (!a_buffer) {
            ...
            _originally_allocated_cols = cols;
            if (_fn) {
                voice_ei_dsp_register_matrix_alloc_internal(fn, file, line, rows, cols, sizeof(int32_t), buffer);
            }
            else {
                voice_ei_dsp_register_matrix_alloc(rows, cols, sizeof(int32_t), buffer);
            }
#endif
        }
    }
 ...
 */
typedef struct voice_ei_quantized_matrix {
    uint8_t *buffer;
    uint32_t rows;
    uint32_t cols;
    bool buffer_managed_by_me;
     ...
     */
    voice_ei_quantized_matrix(uint32_t n_rows,
                        uint32_t n_cols,
#ifdef __MBED__
                        ...
                        )
    {
        if (a_buffer) {
            ...
            buffer = (uint8_t*)voice_ei_calloc(n_rows * n_cols * sizeof(uint8_t), 1);
            buffer_managed_by_me = true;
        }
        rows = n_rows;
        cols = n_cols;
        ...
        if (!a_buffer) {
            ...
            _originally_allocated_cols = cols;
            if (_fn) {
                voice_ei_dsp_register_matrix_alloc_internal(fn, file, line, rows, cols, sizeof(uint8_t), buffer);
            }
            else {
                voice_ei_dsp_register_matrix_alloc(rows, cols, sizeof(uint8_t), buffer);
            }
#endif
        }
    }

    ~voice_ei_quantized_matrix() {
        if (buffer && buffer_managed_by_me) {
            voice_ei_free(buffer);
 ...
 * Freeing happens by calling `delete` on the object or letting the object go out of scope.
 */
typedef struct voice_ei_matrix_u8 {
    uint8_t *buffer;
    uint32_t rows;
    uint32_t cols;
    bool buffer_managed_by_me;
 ...
 *  during execution of
 *  [voice_run_classifier()](https://docs.edgeimpulse.com/reference/voice_run_classifier) or
 *  [voice_run_classifier_continuous()](https://docs.edgeimpulse.com/reference/voice_run_classifier_continuous)).
 ...
 *  `voice_run_classifier()` or `voice_run_classifier_continuous()`) will never request more than a
 ...
 */
typedef struct voice_ei_signal_t {
    /**
    ...
    */
    ...
    /**
     * Total number of samples the user will provide (via get_data).  This value should match either the total number of raw features required for a full window (ie, the window size in Studio, but in samples), OR, if using voice_run_classifier_continuous(), the number of samples in a single slice)
     *  for a new slice (`voice_run_classifier_continuous()`) in order to perform
...
#ifdef __cplusplus
typedef struct voice_ei_feature_t {
    union {
        ei::matrix_t* matrix;
