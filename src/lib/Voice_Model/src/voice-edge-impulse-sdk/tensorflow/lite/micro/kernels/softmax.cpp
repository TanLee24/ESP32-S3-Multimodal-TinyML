...
TfLiteStatus SoftmaxEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {
    case kTfLiteFloat32: {
#if VOICE_EI_TFLITE_DISABLE_SOFTMAX_IN_F32
    ...
    case kTfLiteInt8: {
#if VOICE_EI_TFLITE_DISABLE_SOFTMAX_IN_I8
      MicroPrintf("Type %s (%d) not supported.", TfLiteTypeGetName(input->type),
                  input->type);
      return kTfLiteError;
#endif
      if (output->type == kTfLiteInt8) {
#if VOICE_EI_TFLITE_DISABLE_SOFTMAX_OUT_I8
      MicroPrintf("Type %s (%d) not supported.", TfLiteTypeGetName(input->type),
                  output->type);
      return kTfLiteError;
#endif
...
#if VOICE_EI_TFLITE_DISABLE_SOFTMAX_OUT_I16
      MicroPrintf("Type %s (%d) not supported.", TfLiteTypeGetName(input->type),
                  output->type);
      return kTfLiteError;
#endif
    ...
    case kTfLiteInt16: {
#if VOICE_EI_TFLITE_DISABLE_SOFTMAX_IN_I16
      MicroPrintf("Type %s (%d) not supported.", TfLiteTypeGetName(input->type),
                  input->type);
      return kTfLiteError;
#endif
