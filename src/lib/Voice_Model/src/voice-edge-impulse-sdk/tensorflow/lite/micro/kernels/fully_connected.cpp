...
#include "voice-edge-impulse-sdk/tensorflow/lite/micro/kernels/fully_connected.h"
...
#include "voice-edge-impulse-sdk/tensorflow/lite/kernels/internal/reference/fully_connected.h"
#include "voice-edge-impulse-sdk/tensorflow/lite/kernels/internal/reference/integer_ops/fully_connected.h"
...
#if VOICE_EI_TFLITE_DISABLE_FULLY_CONNECTED_IN_I16
...
#if VOICE_EI_TFLITE_DISABLE_FULLY_CONNECTED_IN_I8
        MicroPrintf("Filter data type %s currently not supported.",
                              TfLiteTypeGetName(input->type));
        return kTfLiteError;
#endif
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {
    case kTfLiteFloat32: {
#if VOICE_EI_TFLITE_DISABLE_FULLY_CONNECTED_IN_F32
...
#define TF_LITE_FULLY_CONNECTED(output_data_type)      \
  ...
  switch (output->type) {
    case kTfLiteUInt8:
      #if VOICE_EI_TFLITE_DISABLE_FULLY_CONNECTED_OUT_U8
      ...
      break;
    case kTfLiteInt16:
      #if VOICE_EI_TFLITE_DISABLE_FULLY_CONNECTED_OUT_I16
      MicroPrintf("Type %s currently not supported.",
                            TfLiteTypeGetName(filter->type));
      return kTfLiteError;
      #endif
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {
    ...
    case kTfLiteUInt8:
      #if VOICE_EI_TFLITE_DISABLE_FULLY_CONNECTED_IN_U8
      MicroPrintf("Type %s (%d) not supported.",
                      TfLiteTypeGetName(input->type), input->type);
      return kTfLiteError;
      #endif
